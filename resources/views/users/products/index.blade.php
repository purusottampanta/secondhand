			...
			<div class="row mar-5">
				<div class="media pad-10 {{ getStatusColor($product->status) }}">
					<div class="media-left hidden-xs">
					    <a href="{{ route('users.products.show', $product->product_slug) }}">
					...
					<div class="media-body">
					    <div class="col-md-10">
					    	<a href="{{ route('users.products.show', $product->product_slug) }}" style="{{ getStatusColor($product->status) == 'bg-primary' ? 'color:#000000' : '' }}" class="hidden-xs">
						    	<div class="col-md-4">
						    		<h4 class="media-heading">{{ $product->product_name }}</h4>
						    	</div>
							    <div class="col-md-2">
							    	{{-- <span>
							    		<i class="fa fa-eye pad-r-5"></i>{{ $product->views }}
							    	</span> --}}
						    ...
						    <div class="hidden-md hidden-sm hidden-lg">
						    	<a href="{{ route('users.products.show', $product->product_slug) }}" class="text-lg">{{ $product->product_name }}</a>
						    	<br>
						    	<small>
								...
								<div class="row pad-l-10">
									{{-- <small>
										<a href="{{ route('users.products.edit', $product->product_slug) }}"><u>Edit Ad</u>
										</a>
									</small> --}}
									...
									@if($product->status == 'sell_request')
										<a href="{{ route('users.products.edit', $product->product_slug) }}" class="btn btn-xs" type="button" title="edit"><i class="fa fa-edit"></i></a>
									    {!! getDeleteForm(route('users.products.destroy', $product->id), 'Delete product?', 'Are you sure you want to delete this product', 'btn btn-flat ink-reaction text-warning', 'fa fa-archive') !!}
									@endif
								</div>
						    </div>
					    </div>
